  ...
  return (
    <div className='container p-4 max-w-5xl'>
              ...
              {jobs.map((job,index)=>(
                <tr key={index} className='text-gray-700'>
                  ...
                  <td className='py-2 px-4 border-b text-center'>
                    <input onChange={() => changeJobVisibility(job._id)} className='scale-125 ml4' type="checkbox" checked={job.visible} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className='mt-4 flex justify-end'>
        <button onClick={()=> navigate("/dashboard/add-job")} className='bg-black cursor-pointer text-white py-2 px-4'>Add New Job</button>
      </div>
    </div>
  )
}
